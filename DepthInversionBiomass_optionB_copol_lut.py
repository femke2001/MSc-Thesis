...
# Roughness-activation coefficient adopted after calibration against the
# normalized I2EM roughness response (Appendix: roughness_gain_calibration).
# Fitted nominal value (eps'=3.0, L/s=6, theta=26 deg, a=1 fixed) was ~2.72;
# adopted/rounded value used throughout the thesis is 2.75. This module uses
# the fitted nominal value as the code default; pass beta=2.75 explicitly if
# you want the rounded, as-reported-in-text constant instead.
ADOPTED_BETA = 2.72
ADOPTED_A = 1.0
...
# =============================================================================
# Product I/O and geometry helpers (from DepthInversionBiomass)
# =============================================================================
...
# =============================================================================
# Shared utilities, dielectric models and Model 1 (Fresnel LUT) forward model
# =============================================================================
...
def invert_eps_ks_from_hh_vv_lut(
    ...
    Implementation note: loops over the (eps, ks) grid keeping only a
    running best-cost / best-index image (O(rows*cols), not
    O(grid*rows*cols)) to avoid building the full prediction stack in
    memory for a full-resolution BIOMASS scene.
...
def invert_eps_ks_from_i2em_lut(
    ...
    *,
    ...
    # --- Bound per-block memory ---------------------------------------------
    # The cost matrix and its transients are (block_size, n_cand). Per channel
    # a few full-size arrays are briefly live (a float64 residual, its float32
    # copy, and the float32 accumulator), so peak memory scales as roughly
    # 16 * block_size * n_cand bytes. The historical default block_size
    # (200000) was chosen for a small candidate grid; with a large LUT grid
    # (n_cand here can reach ~1.8e4) a single block is multiple GiB and
    # overflows RAM (the reported failure was a 2 GiB (30618, 17760) array).
    # Cap block_size against a fixed budget so peak stays bounded for any grid
    # size. Batching only changes how pixels are grouped -- per-pixel argmin is
    # independent of the grouping -- so the retrieved maps are unchanged.
    # Raise _BLOCK_MEM_BUDGET_BYTES on a machine with more free RAM if the
    # extra Python-loop iterations ever matter for speed.
...
# =============================================================================
# Perpendicular baseline from orbit state vectors (from orbit_baseline_from_products)
# =============================================================================
...
def _orb_xml_path_from_root(root_folder) -> Path:
    ...
    return root_folder / "annotation_primary" / "navigation" / f"{base.lower()}_orb.xml"
...
def _state_vector_from_orbit_xml(product, orb_xml_path=None, ref_time_override=None):
    ...
    src = f"annotation_orb_file:OSV[{idx}]@{t_txt}"
